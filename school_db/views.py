...
def problem_four(request):
  ...
  Courses Count: {Course.objects.count()}
  Instructors Count: {Instructor.objects.count()}
...
def problem_five(request):
  new_student = Student.objects.create(first_name='Mara', last_name = 'Green', year='2022', gpa=4.00)
  print(f"""
  Id: {new_student.pk}
...
def problem_six(request):
    ...
    student_id = 11
    updated_info = Student.objects.filter(id=11).update(gpa=4.2)
    find_student = Student.objects.get(pk=student_id)
    print(f"""
    Id: {student_id}
    Full Name: {find_student.first_name} {find_student.last_name}
    GPA: {find_student.gpa}
